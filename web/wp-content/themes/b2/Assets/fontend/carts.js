var carts = new Vue({
    ...
    async mounted(){
        ...
        if(count && count <= 0){
            this.$toasted.show(b2_global.js_text.global.buy_count_error, { 
                theme: 'primary', 
                position: 'top-center', 
                duration : 4000,
                type:'error'
        ...
        if(id){
            ...
            await localforage.getItem('carts').then(value=>{
                let data = value
    ...
    methods:{
        ...
        getItems(id){
            ...
            this.$http.post(b2_rest_url+'getShopItemsData',Qs.stringify(ids)).then((res)=>{
                this.data = res.data
                this.data[id].count = b2GetQueryVariable('count')
                this.init()
            })
        ...
        pay(){
            ...
            if(this.showAddress === true && this.pickedAddress === ''){
                ...
                this.$toasted.show(b2_global.js_text.global.add_address, { 
                    theme: 'primary', 
                    position: 'top-center', 
                    duration : 4000,
                    type:'error'
            ...
            if(this.showEmail === true && this.pickedEmail == ''){
                this.$toasted.show(b2_global.js_text.global.add_email, { 
                    theme: 'primary', 
                    position: 'top-center', 
                    duration : 4000,
                    type:'error'
